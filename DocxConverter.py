...
def format_text_to_html(text):
    ...
    # Вариант 2: Два инициала с точками слитно (Иванов И.И. или Иванов И.И)
    html_text = re.sub(
        r'([А-ЯЁ][а-яё]{1,24}(?:-[А-ЯЁ][а-яё]{1,24})?)\s+([А-ЯЁ])\.([А-ЯЁ])\.?',
        r'\1&nbsp;\2.&nbsp\3.',
        html_text
    )
    
    # Вариант 3: Один инициал с точкой после фамилии (Иванов И.)
    html_text = re.sub(
        r'([А-ЯЁ][а-яё]{1,24}(?:-[А-ЯЁ][а-яё]{1,24})?)\s+([А-ЯЁ])\.',
        r'\1&nbsp;\2.',
        html_text
    )
    
    # Вариант 4: Два инициала без точек через пробел (Иванов А Б)
    html_text = re.sub(
        r'([А-ЯЁ][а-яё]{1,24}(?:-[А-ЯЁ][а-яё]{1,24})?)\s+([А-ЯЁ])\s+([А-ЯЁ])\b',
        r'\1&nbsp;\2&nbsp;\3',
        html_text
    )
    
    # Вариант 5: Один инициал без точки (Иванов А) - только если это конец предложения или перед запятой/точкой
    html_text = re.sub(
        r'([А-ЯЁ][а-яё]{1,24}(?:-[А-ЯЁ][а-яё]{1,24})?)\s+([А-ЯЁ])(?=[\s,\.;:!?])',
        r'\1&nbsp;\2',
        html_text
    )
