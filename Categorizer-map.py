...
def DoFolder(source_folder, table, dest_folder):
    ...
    for sheet_name in wb.sheetnames:
        ...
        if subfolder_name_cell:
            ...
            map[folder_name][subfolder_name] = []
            folder_data = map[folder_name][subfolder_name]
        else:
            subfolder_name = ""
        ...
        for row in ws.iter_rows(min_row=4, values_only=True):
            ...
            normalized_table_entry = normalize_filename(image_name)
            ...
            if normalized_table_entry in normalized_dict:
                ...
                map[folder_name][normalized_table_entry] = []
                map[folder_name][normalized_table_entry]["description"] = description
                map[folder_name][normalized_table_entry]["title"] = image_name
...
def main():
    ...
    sortedPath = resolve_relative_path("SortedMap")
    ...
    DoFolder(
        ...
        r"SortedMap\Peterhof\разрушения"
    )
    logger.debug(f"##################################################################################################################")
    ...
    DoFolder(
        ...
        r"SortedMap\Peterhof\восстановление"
    )
